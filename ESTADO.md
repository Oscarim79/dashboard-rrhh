...
- **Propuesta = una sola (Oscar, 2026-09-16, segunda ronda):** ya no hay "Alternativa 2" frente a cuatro
  propuestas. La Sección 2 se llama "La propuesta" y contiene solo el reparto discutido con Gerencia (chip
  "Discutida con Gerencia · pendiente de aprobación"): tabla Necesidad → Qué se hará, "Por qué conviene" y "Lo que
  queda por definir al aprobarla", seguido de "Detalle A" (capacitador comercial por región, sigue sirviendo
  `?solo=capacitador`) y "Detalle B" (métrica de cultura). Se retiraron las antiguas propuestas 1 (asistente para
