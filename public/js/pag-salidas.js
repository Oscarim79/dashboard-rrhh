...
if (!salidasTodo.total) {
  ...
  function pintarTodo(depto) {
    ...
    elegir(anios.includes(periodoActual) || periodoActual === 'todo' ? periodoActual : 'todo');
  }

  const deptoInicial = pintarSelectorDepto((d) => pintarTodo(d));
  pintarTodo(deptoInicial);
  pintarPie(meta);
}
